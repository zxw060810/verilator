...
& "$envRoot\Scripts\verilator.bat" --binary --Mdir $objB -CFLAGS -O2 -CFLAGS -std=gnu++20 --top-module tb_badsolver ci_win32/tb_badsolver.sv *> "$WorkDir\ci_win32\log_e2_verilate_bad.txt"
$vlExit3 = $LASTEXITCODE
Write-Output "  verilate exits: combos=$vlExit free=$vlExit2 badsolver=$vlExit3 (non-zero expected: runner has a make on PATH, auto-make fails on cl env, files are still generated)"
...
if (-not (Test-Path "$WorkDir\$objB\Vtb_badsolver.mk"))  { Die "badsolver mk not generated" }
...
mingw32-make -C $objB -f Vtb_badsolver.mk CXX=g++ "LINK=g++ -static" AR=ar PYTHON3=python VERILATOR_ROOT=$vrRoot -j 4 *> "$WorkDir\ci_win32\log_e3_make_bad.txt"
if ($LASTEXITCODE -ne 0) { Die "mingw make badsolver failed" }
...
Write-Output "=== E7b: bad-solver WITH constraint (expect warning + randomize()==0 + clean exit) ==="
$env:VERILATOR_SOLVER = "no_such_solver_xyz"
